...
| Plans | `plans/` | Implementation plans, research, agent reports, chronological journals |
...
├── journals/                  # Chronological work records; not current authority
...
```bash
date +%y%m%d-%H%M   # run_shell capability
```
...
|----------|---------|---------|
| Single file output | Flat file in category dir | `plans/journals/260304-session-review.md` |
...
| Variant-based | Flat with variant suffix | `logo-light.svg`, `logo-dark.svg` |
...
3. Is it an implementation plan or agent output?
   ...
   → Technical journal: plans/journals/{date-slug}.md
   → NO: continue
...
When invoked directly with `/ak:project-organization [targets]`:
